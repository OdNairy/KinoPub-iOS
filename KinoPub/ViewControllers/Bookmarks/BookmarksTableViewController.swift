...
class BookmarksTableViewController: UITableViewController, SideMenuItemContent {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(
            UINib(nibName: String(describing: BookmarkTableViewCell.self), bundle: Bundle.main),
    ...
    func configHint() {
        let tableViewFooter = UIView(
            frame: CGRect(x: 0, y: 0, width: tableView.frame.width, height: 66))
        tableViewFooter.backgroundColor = .clear
        let label = UILabel(
            frame: CGRect(x: 15, y: 10, width: tableView.frame.width - 30, height: 46))
        label.font = label.font.withSize(12)
        label.text =
            "Для удаления папки просто проведите по ней справа налево. Также вы можете переносить фильмы из одной папки в другую."
    ...
    func showNewFolderAlert() {
        ...
        Alert(title: "Новая папка", message: "Придумайте короткое и ёмкое название для новой папки")
            ...
            .addAction("Создать", style: .default, preferredAction: true) { [weak self] (_) in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath)
        ...
        let cell =
            tableView.dequeueReusableCell(
                withIdentifier: String(describing: BookmarkTableViewCell.self), for: indexPath)
...
extension BookmarksTableViewController {
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let deleteAction = UIContextualAction(style: .destructive, title: "Удалить") { [weak self] (_, _, completionHandler) in
            guard let strongSelf = self else { return }
            let folder = strongSelf.viewModel.bookmarks[indexPath.row].id
