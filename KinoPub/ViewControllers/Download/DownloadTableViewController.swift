...
class DownloadTableViewController: UITableViewController, SideMenuItemContent {
    ...
    func config() {
        ...
        tableView.register(
            UINib(nibName: String(describing: DownloadingTableViewCell.self), bundle: Bundle.main),
            forCellReuseIdentifier: DownloadingTableViewCell.reuseIdentifier)
        tableView.register(
            UINib(nibName: String(describing: DowloadedTableViewCell.self), bundle: Bundle.main),
            forCellReuseIdentifier: DowloadedTableViewCell.reuseIdentifier)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath)
        ...
        switch indexPath.section {
            case 0:
                let cell =
                    self.tableView.dequeueReusableCell(
                        withIdentifier: DownloadingTableViewCell.reuseIdentifier, for: indexPath)
                    as! DownloadingTableViewCell
            ...
            case 1:
                let cell =
                    self.tableView.dequeueReusableCell(
                        withIdentifier: DowloadedTableViewCell.reuseIdentifier, for: indexPath)
                    as! DowloadedTableViewCell
...
extension DownloadTableViewController {
    func showActionController(at indexPath: IndexPath) {
        ...
        if downing[selectedIndexPath.row].status == .NTDownloading {
            action.addAction(
                "Пауза", style: .default,
                handler: { (_) in
                    NTDownloadManager.shared.pauseTask(
                        downloadTask: self.downing[self.selectedIndexPath.row])
                })
        }
        if downing[selectedIndexPath.row].status == .NTPauseDownload {
            action.addAction(
                "Продолжить", style: .default,
                handler: { (_) in
                    NTDownloadManager.shared.resumeTask(
                        downloadTask: self.downing[self.selectedIndexPath.row])
                })
        }
    ...
    func showConfirmAlert() {
        Alert(message: "Удалить?")
            .tint(.kpBlack)
            .addAction("Да", style: .destructive) { (_) in
                NTDownloadManager.shared.removeTask(
                    downloadTask: self.downing[self.selectedIndexPath.row])
...
extension DownloadTableViewController: NTDownloadManagerDelegate {
    func downloadRequestFinished(downloadTask: NTDownloadTask) {
        let title = downloadTask.fileName.replacingOccurrences(of: ".mp4", with: "")
            .replacingOccurrences(of: ";", with: "").replacingOccurrences(of: ".", with: " ")
        let banner = NotificationBanner(
            title: "Завершено", subtitle: "\(title) успешно загружен", style: .success)
        banner.duration = 3
    ...
    func downloadRequestDidFailedWithError(error: Error, downloadTask: NTDownloadTask) {
        ...
        Alert(
            title: "Ошибка",
            message: "При загрузке \(title) произошла ошибка: \(error.localizedDescription)"
        )
        .showOkay()
        initdata()
    }
