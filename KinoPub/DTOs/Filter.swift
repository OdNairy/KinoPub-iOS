...
struct Filter: ReflectedStringConvertible {
    ...
    static var defaultFilter: Filter {
        let filter = Filter(
            genres: nil, countries: nil, subtitles: nil, year: nil, yearsDict: nil,
            sort: SortOption.updated, sortAsc: false)
        return filter
    }

    var isSet: Bool {
        if genres != nil || countries != nil || subtitles != nil || year != nil
            || sort != SortOption.updated || sortAsc {
            return true
        }
        return false
    }
            ...
            switch year {
                ...
                case "Период":
                    if var yearsDict = yearsDict {
                        if let year1 = yearsDict["from"]?.int, let year2 = yearsDict["to"]?.int,
                            year1 > year2 {
