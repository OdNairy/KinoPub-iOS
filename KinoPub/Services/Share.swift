...
class Share {
        ...
        func convert(url: String, title: String? = nil) -> URL? {
            switch self {
                case .VLC:
                    ...
                    if let title = title,
                        let encodedString = title.addingPercentEncoding(
                ...
                case .nPlayer:
                    return URL(
                        string: url.replacingOccurrences(of: "https://", with: "nplayer-http://"))
            }
        }
    ...
    func showActions(
        ...
        let action = ActionSheet()
            ...
            .addAction(
                "Открыть в", style: .default,
                handler: { (_) in
                    self.openInAppScheme(
                        url: url, title: title, quality: quality, inView: view, forButton: button)
                }
            )
            .addAction(
                "Копировать ссылку", style: .default,
                handler: { (_) in
    ...
    func openInAppScheme(
        url: String, title: String, quality: String, inView view: UIView?,
        forButton button: UIBarButtonItem?
    ) {
        let action = ActionSheet()
            .tint(.kpBlack)
            .addAction(
                "Открыть в VLC", style: .default,
                handler: { (_) in
                    self.open(url: url, player: .VLC)
                }
            )
            .addAction(
                "Открыть в Infuse", style: .default,
                handler: { (_) in
                    self.open(url: url, player: .Infuse, pasteboardValue: url)
                }
            )
            .addAction(
                "Открыть в Documents", style: .default,
                handler: { (_) in
                    self.open(
                        url: url,
                        player: .Documents,
            ...
            .addAction(
                "Открыть в nPlayer", style: .default,
                handler: { (_) in
