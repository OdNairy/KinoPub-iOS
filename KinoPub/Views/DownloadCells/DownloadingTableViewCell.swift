...
class DownloadingTableViewCell: UITableViewCell {
    ...
    var fileInfo: NTDownloadTask? {
        didSet {
            ...
            guard let downloadedFileSize = fileInfo?.downloadedFileSize,
            ...
            progressLabel.text = String(
                format: "  \(downSize) %@ / \(size) %@  ", downloadedFileSize.size,
                downloadedFileSize.unit, fileSize.size, fileSize.unit)
    ...
    func configView() {
        ...
        progressView.centerImage = UIImage(named: "Download Start")?.imageWithInset(
            UIEdgeInsets(top: 36, left: 36, bottom: 36, right: 36))?.withRenderingMode(
                .alwaysTemplate
            ).tint(.kpMarigold, blendMode: CGBlendMode.normal)
    ...
    func changeIcon() {
        if fileInfo?.status == NTDownloadStatus.NTPauseDownload {
            ...
            progressView.centerImage = UIImage(named: "Download Pause")?.imageWithInset(
                UIEdgeInsets(top: 36, left: 36, bottom: 36, right: 36))?.withRenderingMode(
                    .alwaysTemplate
                ).tint(.kpMarigold, blendMode: CGBlendMode.normal)
        }
