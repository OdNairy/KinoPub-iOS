    ...
    func configure(withItem item: Item) {
        ...
        if let kinopoiskId = item.kinopoisk {
            var userinfo: [AnyHashable: Any] = [
                "message":
                    "Рейтинг КИНОПОИСК \(kinopoiskRatingLabel.text ?? "-") из 10\n Проголосовали \(item.kinopoiskVotes?.string ?? "-") человек"
            ]
        ...
        if let ratingVotes = item.ratingVotes {
            let userinfo: [AnyHashable: Any] = [
                "message":
                    "Рейтинг Кинопаба \(kinopubRatingLabel.text ?? "-") (\(item.ratingPercentage ?? "-")%)\n Проголосовали \(ratingVotes.string) человек"
            ]
            tapped(view: kinopubRatingView, userinfo: userinfo)
        }

        let userinfo: [AnyHashable: Any] = [
            "message": "Количество просмотров данного видео в сервисе кинопаб"
        ]
