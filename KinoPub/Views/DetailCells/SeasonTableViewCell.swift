...
class SeasonTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var episodesCollectionView: UICollectionView! {
        didSet {
            episodesCollectionView.contentInset = UIEdgeInsets(
                top: 0, left: 15, bottom: 0, right: 8)
        }
    }
    ...
    override func awakeFromNib() {
        ...
        episodesCollectionView.register(
            UINib(
                nibName: String(describing: EpisodesCollectionViewCell.self), bundle: Bundle.main),
...
extension SeasonTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath)
        ...
        let cell =
            self.episodesCollectionView.dequeueReusableCell(
                withReuseIdentifier: String(describing: EpisodesCollectionViewCell.self),
            ...
            case UICollectionView.elementKindSectionHeader:
                let sectionHeaderView =
                    collectionView.dequeueReusableSupplementaryView(
                        ofKind: kind,
                        withReuseIdentifier: String(describing: SeasonHeaderView.self),
                        for: indexPath) as! SeasonHeaderView
