...
class CustomAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        self.isPresenting
            ? container.addSubview(toView) : container.insertSubview(toView, belowSubview: fromView)

        let detailView = isPresenting ? toView : fromView

        guard let artwork = detailView.viewWithTag(CustomAnimatorTag) as? UIImageView else {
            return
        }
        artwork.frame.origin.y += 220
        artwork.image = image
        artwork.alpha = 0
        ...
        toView.frame =
            isPresenting
            ? CGRect(
                x: fromView.frame.width, y: 0, width: toView.frame.width,
        ...
        UIView.animate(
            ...
            animations: {
                ...
                detailView.frame =
                    ...
                    ? fromView.frame
                    : CGRect(
                        x: toView.frame.width, y: 0, width: toView.frame.width,
