...
class MediaManager {
    ...
    init() {
        NotificationCenter.default.addObserver(
            self, selector: #selector(updateDisplayFromDefaults),
            name: UserDefaults.didChangeNotification, object: nil)

        // Custom
        NotificationCenter.default.addObserver(
            self, selector: #selector(playerDidPlayToEnd(_:)),
            name: NSNotification.Name.EZPlayerPlaybackDidFinish, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(playerTimeDidChange(_:)),
            name: NSNotification.Name.EZPlayerPlaybackTimeDidChange, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(playerStatusDidChange(_:)),
            name: NSNotification.Name.EZPlayerStatusDidChange, object: playerCustom)

        // Native
        NotificationCenter.default.addObserver(
            self, selector: #selector(playerDidPlayToEnd(_:)),
            name: NSNotification.Name.AVPlayerItemDidPlayToEndTime,
            object: playerNative?.currentItem)
        NotificationCenter.default.addObserver(
            self, selector: #selector(playerDidClosed(_:)),
            name: NSNotification.Name.DTSPlayerViewControllerDismissed, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(playerTimeDidChange(_:)),
            name: NSNotification.Name.DTSPlayerPlaybackTimeDidChange, object: nil)
    }
    ...
    private func addPlayerItemTimeObserver() {
        self.timeObserver = self.playerNative?
            .addPeriodicTimeObserver(
                forInterval: CMTimeMakeWithSeconds(
                    Config.shared.delayViewMarkTime,
    ...
    func playVideo(
        mediaItems: [MediaItem], userinfo: [AnyHashable: Any]? = nil, isLive: Bool = false
    ) {
        releaseNativePlayer()
        releasePlayer()
        self.isLive = isLive
        ...
        Defaults[.isCustomPlayer]
            ? playWithCustomPlayer(mediaItems: mediaItems, userinfo: userinfo)
               ...
               let index = strongSelf.playerItems.firstIndex(of: item),
                ...
                Alert(message: "Продолжить с \(timeToSeek.timeIntervalAsString("hh:mm:ss"))?")
                    ...
                    .addAction(
                        "Да", style: .default,
                        handler: { (_) in
                            strongSelf.fullScreenViewController?.player?.seek(
                                to: CMTime(seconds: timeToSeek, preferredTimescale: 1))
                        }
                    ).show(animated: true)
            }
        }
    ...
    func playWithCustomPlayer(mediaItems: [MediaItem], userinfo: [AnyHashable: Any]? = nil) {
        ...
        if mediaItems.first?.title != nil {
            self.playerCustom!.playWithURL(
                (mediaItems.first?.url)!, embeddedContentView: nil, title: mediaItems.first?.title)
        } else {
    ...
    func releaseNativePlayer() {
        if self.fullScreenViewController != nil {
            self.fullScreenViewController?.dismiss(
                animated: true,
                completion: {
    ...
    func configVolumeView() {
        ...
        volume?.frame = CGRect(
            x: 0, y: 20, width: (playerCustom?.view.frame.size.width)!, height: 2)
        ...
        if let item = playerNative?.currentItem, let index = playerItems.firstIndex(of: item) {
            if let id = mediaItems[index].id, let video = mediaItems[index].video {
                logViewsManager.changeMarktime(
                    ...
                    season: mediaItems[index].season)
                self.time = Date().timeIntervalSinceReferenceDate
            }
        } else if playerCustom != nil, let id = mediaItems[0].id, let video = mediaItems[0].video {
            logViewsManager.changeMarktime(
                id: id, time: (playerCustom?.currentTime)!, video: video,
                season: mediaItems[0].season)
            self.time = Date().timeIntervalSinceReferenceDate
        }
